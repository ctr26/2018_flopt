...
Typically a Radon transform used to then transform this set of images into a 3D image stack in Cartesian coordinates (\(X,Y,Z\)).
...
The proposed algorithm works by triangulating points between images pairs to extract camera pose using the theoretical framework used in stereoscopic imaging. %TODO reword
...
The stereoscopic imaging of scenes enables the triangulation of individual features in three dimensional space (know as world points) when the features or fiducial markers in one view are uniquely identifiable, see Figure~\ref{fig:epi-polar-geom} for the coordinate system which describes this geometry. %(such as fluorescent beads).
...
During an ideal \gls{OPT} acquisition, a marker will appear to follow an elliptical path in the \(xy\) image plane; and for the volume reconstruction procedure there is a fitting step to recover the path of the fiducial marker, which is used to correct the sinogram before applying the inverse \gls{Radon transform}.
...
Once \gls{F} is calculated it is then decomposed into \(\gls{R}_n\) and \(\gls{T}_n\) between each view \(n\) and \(n+1\).
...
The recovered transformation matrices then matrix inverted and applied to the back projection of the image to align realign the rays in the volume to their respective source positions. %TODO hard sentence.
...
\end{align}
To find the correct matrix between the \(n=0\) and \(n=1\), each of the four are compared to an ideal matrix which is composed using \emph{a priori} knowledge of the likely angle of rotation of the system's imaging properties.
...
The reference image was then rotated through \SI{128} angles over \(2\pi \) radians and projected along the \(Y\) axis; then an image slice in (\(X,Y\)) was taken to create a single line projection, shown three dimensionally in \figurename~\ref{fig:recon_iterative}.
